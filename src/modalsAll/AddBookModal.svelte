<script lang="ts">
    ...
    async function create() {
        ...
        try {
            const res = await bookEndpoint.post(+room.value, +person.value, +price.value, booker_name.value, bookDate, token)
...
<div class={"h-screen w-screen bg-black/70 fixed top-0 left-0 bottom-0 right-0 z-50 justify-center items-center " + (show ? "flex" : "hidden")}>
    ...
    <div class="bg-white p-8 flex flex-col gap-3 w-screen md:w-1/3 h-full md:h-[fit-content] md:rounded-md shadow-md overflow-y-auto">
        <p class="text-xl text-center font-bold">Bron qilish</p>
        <div class="flex flex-col gap-3">
            <div class="room_id flex flex-col gap-2">
                <label class="font-semibold" for="room">Xonani tanlang*:</label>
                <select bind:this={room} class="outline-0 border-2 px-3 py-1 rounded" name="room">
            ...
            <div class="boked_date flex justify-between">
                <div class="flex flex-col gap-2">
                    ...
                    <input bind:this={booked_date} on:change={() => {console.log(new Date(booked_date.value).toLocaleDateString())}} class="outline-0 border-2 px-3 py-1 rounded" type="date" name="" id="">
                </div>
                <div class="flex flex-col gap-2">
                    <label class="font-semibold" for="person">Vaqt*:</label>
                    <select bind:this={booked_time} class="outline-0 border-2 px-3 py-1 rounded" name="" id="">
            ...
            <div class="person flex flex-col gap-2">
                ...
                <input bind:this={person} class="outline-0 border-2 px-3 py-1 rounded" type="text" name="" id="">
            </div>
            <div class="price flex flex-col gap-2">
                <label class="font-semibold" for="">Bron narxi*:</label>
                <input bind:this={price} class="outline-0 border-2 px-3 py-1 rounded" type="text" name="" id="">
            </div>
            <div class="booker_name flex flex-col gap-2">
                <label class="font-semibold" for="">Mijoz ismi*:</label>
                <input bind:this={booker_name} class="outline-0 border-2 px-3 py-1 rounded" type="text" name="" id="">
            </div>
        </div>
