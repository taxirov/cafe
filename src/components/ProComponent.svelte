...
<div class="flex flex-col gap-3 bg-white p-5 rounded-md">
    ...
    {#if proInOrder.order.room != null}
        <p class="text-lg md:text-xl"><b>Xona:</b> {proInOrder.order.room.name}</p>
    {:else}
        <p class="text-lg md:text-xl"><b>Xona:</b> SOBOY/DOSTOVKA</p>
    {/if}
    <div class="flex justify-between">
        <button on:click={() => { showDeletePro = true }} class="bg-red-500 text-white text-sm py-2 px-3 rounded-md font-semibold w-fit">Bekor qilish <i class="bi bi-x"></i></button>
        <button on:click={() => { showEditStatus = true }} class="bg-green-500 text-white text-sm py-2 px-3 rounded-md font-semibold w-fit">Tasdiqlash <i class="bi bi-check"></i></button>
    </div>
</div>