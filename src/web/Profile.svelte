...
<section class="flex min-h-screen md:h-screen">
    ...
    <div class="grow left-0 flex flex-col h-screen">
        <div class="grow-0 flex justify-between items-center sticky top-0 left-0 right-0 bg-indigo-500 p-3 h-fit">
            <h2  class="outline-none text-2xl font-bold text-zinc-100"><i class="bi bi-person-fill"></i> Profile</h2>
        </div>
        <div class="grow flex flex-col justify-between p-5 overflow-y-auto">
            <div class="flex gap-5">
                <div class="flex flex-col gap-3 w-2/3">
                    <div class="flex justify-between items-center bg-white/80 rounded-xl py-3 px-4 shadow-sm">
                        <p class="font-bold text-xl"><i class="bi bi-at"></i> Username:</p>
                        <p class="font-medium text-2xl">{user.username}</p>
                    </div>
                    <div class="flex justify-between items-center bg-white/80 rounded-xl py-3 px-4 shadow-sm">
                        <p class="font-bold text-xl"><i class="bi bi-person"></i> Ism Familiya:</p>
                        <p class="font-medium text-2xl">{user.name}</p>
                    </div>
                    <div class="flex justify-between items-center bg-white/80 rounded-xl py-3 px-4 shadow-sm">
                        <p class="font-bold text-xl"><i class="bi bi-shield-check"></i> Rol:</p>
                        <p class="font-medium text-2xl">{user.role.toLocaleUpperCase()}</p>
                    </div>
                    <div class="flex justify-between items-center bg-white/80 rounded-xl py-3 px-4 shadow-sm">
                        <p class="font-bold text-xl"><i class="bi bi-phone"></i> Telefon raqam:</p>
                        <p class="font-medium text-2xl">{user.phone}</p>
                    </div>
                    <div class="flex justify-between items-center bg-white/80 rounded-xl py-3 pl-4 pr-2 shadow-sm">
                        <p class="font-bold text-xl"><i class="bi bi-envelope"></i> Email:</p>
                        <button on:click={copyEmail} class="font-medium text-md bg-slate-200 px-2 py-1 rounded-xl">{user.email}</button>
                    </div>
                    <div class="flex justify-between items-center bg-white/80 rounded-xl py-3 px-4 shadow-sm">
                        <p class="font-bold text-xl"><i class="bi bi-cash"></i> Oylik maosh:</p>
                        <p class="font-medium text-2xl">{user.salary.toLocaleString('ja-JP')} so'm</p>
                    </div>
                    <div class="flex justify-between items-center bg-white/80 rounded-xl py-3 px-4 shadow-sm">
                        <p class="font-bold text-xl"><i class="bi bi-calendar-event"></i> Ro'yhatdan o'tgan sana:</p>
                        <p class="font-medium text-2xl">{user.create_date.toString().split('T')[0]}</p>
                    </div>
                </div>
            ...
            <div class="flex justify-center items-center bg-red-500 text-white rounded-xl py-3 shadow-sm">
                <button on:click={logOut} class="font-bold text-xl"><i class="bi bi-box-arrow-left"></i>  Tizimdan chiqish</button>
            </div>
        </div>
    </div>
</section>
