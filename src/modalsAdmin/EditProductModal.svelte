<script lang="ts">
    ...
    async function edit() {
        let id = product.id
        try {
            const res = await productEndpoint.put(id, +category_id.value, name.value, +price.value, desc.value, token, admin_key)
...
<div class={"h-screen w-screen bg-black/70 fixed top-0 left-0 bottom-0 right-0 z-[999] justify-center items-center " + (show ? "flex" : "hidden")}>
    ...
    <div class="bg-white p-8 flex flex-col gap-3 w-screen h-full md:h-[fit-content] md:w-[fit-content] md:rounded-md shadow-md overflow-y-auto">
        ...
        <p class="text-xl text-center font-bold">Mahsulotni tahrirlash</p>
        ...
        <div class="flex flex-col gap-3">
            ...
            <div class="flex flex-col gap-2">
                ...
                <input bind:this={name} value="{product.name}"  class="outline-0 border-2 px-3 py-1 rounded" type="text" name="" id="" placeholder="Coca Cola 1.5">
            </div>
            <div class="flex flex-col gap-2">
                <label class="font-semibold" for="desc">Narxi*:</label>
                <input bind:this={price} value="{product.price}"  class="outline-0 border-2 px-3 py-1 rounded" type="text" name="" id="" placeholder="10000">
            </div>
            <div class="flex flex-col gap-2">
                <label class="font-semibold" for="desc">Izoh*:</label>
                <textarea bind:this={desc} value="{product.desc}" class="outline-0 border-2 px-3 py-1 rounded" name="desc" id="" rows="4" placeholder="izoh uchun joy"></textarea>
            </div>
            <div class="flex flex-col gap-2">
